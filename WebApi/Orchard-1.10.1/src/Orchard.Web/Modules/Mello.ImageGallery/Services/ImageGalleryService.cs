...
namespace Mello.ImageGallery.Services
{
    public class ImageGalleryService : IImageGalleryService
    {
        ...
        private const string OnGalleryImagesChanged = "OnGalleryImagesChanged";
        ...
        public ImageGalleryService(IMediaLibraryService mediaService, IRepository<ImageGallerySettingsRecord> repository,
                                   ...
                                   IStorageProvider storageProvider, ICacheManager cacheManager, ISignals signals)
        {
        ...
        public Models.ImageGallery GetImageGalleryPaged(string imageGalleryName, PagerParameters pagerParameters)
        {
            if (imageGalleryName.Contains("\\") || imageGalleryName.Contains("/"))
                imageGalleryName = GetName(imageGalleryName);
            ...
            if (mediaFolder != null)
            {
                return CreateImageGalleryFromMediaFolderPaged(mediaFolder, pagerParameters);
            }
            return null;
        }
        ...
        {
            _signals.Trigger(OnGalleryImagesChanged);
        ...
        private Models.ImageGallery CreateImageGalleryFromMediaFolder(IMediaFolder mediaFolder)
        {
            var images = _cacheManager.Get(mediaFolder.MediaPath, context =>
            {
                context.Monitor(_signals.When(OnGalleryImagesChanged));
                return _mediaService.GetMediaFiles(mediaFolder.MediaPath);
            });
