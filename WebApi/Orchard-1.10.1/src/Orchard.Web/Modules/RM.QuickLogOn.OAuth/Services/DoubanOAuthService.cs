...
{
    public interface IDoubanOAuthService : IDependency
    {
    ...
    public class DoubanOAuthService : IDoubanOAuthService
    {
        public readonly string TokenRequestUrl = "https://www.douban.com/service/auth2/token?client_id={0}&client_secret={1}&code={2}&redirect_uri={3}&grant_type=authorization_code";
            ...
            try
            {
                ...
                var redirectUrl =
                    new Uri(wc.HttpContext.Request.Url,
                            urlHelper.Action("Auth", "DoubanOAuth", new { Area = "RM.QuickLogOn.OAuth" })).ToString();
        ...
        public QuickLogOnResponse Auth(WorkContext wc, string code, string error, string returnUrl)
        {
            ...
            return new QuickLogOnResponse { Error = T("豆瓣登录失败: {0}", error), ReturnUrl = returnUrl };
        }
    }
}
